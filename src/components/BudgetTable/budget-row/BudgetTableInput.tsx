...
const BudgetTableInput = ({
  dollar,
  cents,
  onAssignedChanged,
}: BudgetTableInputProps) => {
  const [value, setValue] = useState(
    `${dollar}.${cents > 9 ? cents : "0" + cents}`
  );
  const [prevValue, setPrevValue] = useState(value);
  const [isSubmitted, setIsSubmitted] = useState(true);

  const updateAssignedDollar = useBudgetStore(
    (state) => state.updateAssignedDollar
  );
  const updateAssignedCents = useBudgetStore(
    (state) => state.updateAssignedCents
  );
  ...
  const handleSubmit = () => {
    const newValue = (parseFloat(value) - parseFloat(prevValue))
      .toFixed(2)
      .toString();
    ...
    onAssignedChanged({
      dollar: parseInt(dollarValue),
      cents: parseInt(centsValue),
    });
  };
